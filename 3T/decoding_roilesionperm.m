function decoding_roilesionperm(sub,analysis_all,rois,shift,condition)
...
for i=1:length(rois)
    ...
    for con_i=1:length(condition)
        ...
        if strcmp(roi,'Amy8_align')
            mask=get_filenames_afni([datafolder sub '/mask/' roi '*freesurfer+orig.HEAD']);
            gm=get_filenames_afni([datafolder sub '/mask/' 'Amy' '_' condition{con_i} '_l2_interacc+orig.HEAD']);
        else
            mask=get_filenames_afni([datafolder sub '/mask/' roi '+orig.HEAD']);
            gm=get_filenames_afni([datafolder sub '/mask/' roi '_' condition{con_i} '_l2_interacc+orig.HEAD']);
        end
        % get number of voxels              
        nv=read_image('AFNI',read_header('AFNI',gm));
        nv=sum(sum(sum(nv)));
        % for each permutaion
        for suf=1:nper        
        ...
        cfg.analysis = 'roi';        
        test=[decode '_' con '_' num2str(suf) '_' roi];

        % Set the output directory where data will be saved, e.g. '/misc/data/mystudy'
        cfg.results.dir = [datafolder sub '/' sub '.' analysis '.results/mvpa/' cfg.analysis '_roilesionperm' '_shift' strrep(num2str(shift), ' ', '') '/' test];
    ...
    %     cfg.design.test(invisible==0,:) = 0;    
        ...
        % remove nv voxels
        idx=randperm(length(passed_data.mask_index));
        passed_data.mask_index=passed_data.mask_index(idx(1:nv));
        passed_data.data=passed_data.data(:,idx(1:nv));
        % Run decoding
