function decoding_roilesiontest(sub,analysis_all,rois,shift,condition)
...
for i=1:length(rois)
    ...
    % if suf start with p
    if strcmp(suf(1),'p')
        cfg.files.mask = maskinter;
    else
        cfg.files.mask = maskroi;
    end    
...
%     cfg.design.nonindependence = 'ok';
    ...
    cfg.design.train(traintrial==0,:) = 0;
    cfg.design.test(traintrial==1,:) = 0;
    switch con
        case 'vis'
    ...
    % change test data if suf start with l
    lesion=[];
    if strcmp(suf(1),'l')
        cfgn=cfg;
        cfgn.files.mask = maskinter;
        lesion=find(load_mask(cfgn)==1);
    end 
