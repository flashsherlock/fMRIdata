...
params:
    ...
    roiname: !r c('all_Amy8_align','all_Amy8_at165')
      ...
      for (r in roi) {
        ...
        if (length(select)>1){
          ...
          if (length(select)==dim(current_data)[2]){
            ...
            select_ext <- rep(rep(select,each=(dim(current_data)[2]/60)),15)
            current_data <- current_data[,select_ext==1]
          }
        }
...
if (!str_detect(mvpa_pattern,"trans")) {
  ...
  for (m in mvpa_pattern){
    ...
    # ttest
    for (r in roi) {
      datashose <- subset(acc4odor,roi==r,select = c("sub","odor","acc"))
      datashose <- spread(datashose,odor,acc)
      # change datachosen to wide format for ttest
      # datashose <- reshape2::dcast(datashose,sub~odor,value.var = "acc")
      cat(r)
      bruceR::TTEST(datashose,c("inc","con"),paired = T)
    }
    # Bar plot of mean +/-se
    control <- subset(acc4odor,roi == params$roi[1])
    control$acc <- 50
