...
params:
    ...
    roi: !r c('Amy8_align','Indiv40')
    roiname: !r c("Amy","Interaction")
...
readlabel <- function(path,sub,analysis,mvpa_pattern,roi_order) {
  ...
  for (ana in analysis) {
    ...
    for (m in mvpa){
      ...
      for (r in roi_order){
        # read mat file
        true <- readMat(file.path(workingdir,m,paste0("odors_",r),'res_true_labels.mat'))
        predict <- readMat(file.path(workingdir,m,paste0("odors_",r),'res_predicted_labels.mat'))
  ...
  for (ana in analysis) {
    for (m in mvpa) {
      for (r in roi) {
        ...
        acc_run[nrow(acc_run)+1,] <- c(sub_i,ana,m,r,paste0("odors_",r),acc,"run_all")
        ...
        for (i_run in 1:run_num) {
          ...
          acc_run[nrow(acc_run)+1,] <- c(sub_i,ana,m,r,paste0("odors_",r),acc,paste("run_",i_run))
        }
      }
    }
  }
