...
# remove zeros
avgdata <- avgdata[4:nrow(avgdata),]
snum <- nrow(avgdata)
row.names(avgdata) <- sprintf("S%02d",c(4:(3+snum)))
...
# remove zeros
rundata <- rundata[,-c(1:30)]
rundata <- cbind(rundata,paste("run",c(1:6),sep = "_"))
names(rundata) <- c(paste0(rep(sprintf("S%02d_",c(4:(3+snum))),each=10),
