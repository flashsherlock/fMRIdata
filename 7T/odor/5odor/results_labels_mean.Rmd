...
roi_select <- params$roi[c(1,6)]
boxset <- function(data){
      summarise(data,
                y0 = quantile(acc, 0.05, na.rm = T), 
                #y0 = mean(acc, na.rm = T)-ci90(acc),
                y25 = quantile(acc, 0.25, na.rm = T), 
                # y50 = median(acc, na.rm = T),
                y50 = mean(acc, na.rm = T),
                y75 = quantile(acc, 0.75, na.rm = T), 
                #y100 = mean(acc, na.rm = T)+ci90(acc))
                y100 = quantile(acc, 0.95, na.rm = T))
}
boxplot <- function(data, select, colors){
  # select data
  Violin_data <- subset(data, roi%in%select,select = c("sub","roi","acc"))
  Violin_data$roi <- factor(Violin_data$roi,levels = select,labels = select)
  # ttest
  # bruceR::TTEST(reshape2::dcast(Violin_data,sub~roi,value.var = "acc"),
  #               y=roi_select,paired = T)
  # detach(package:plyr)
  # library(plyr)
  ...
  ggplot(data = Violin_data, aes(x = roi)) +
    ...
    geom_boxplot(
      ...
      stat = "identity") +
    geom_line(aes(x = con, y = acc, group = sub),color = "gray", show.legend = F) +
    scale_color_manual(values = colors)+
    scale_y_continuous(name = "Accuracy", expand = expansion(add = c(0, 0)))
}
accbox <- boxplot(acc4odor,roi_select,concolor[c(1,5)])+
  coord_cartesian(ylim = c(10,40))+
  scale_x_discrete(labels=c("Amygdala","Piriform"))
ggsave(paste0(params$path,paste0('figures/acc.pdf')), accbox, width = 4, height = 3,
         device = cairo_pdf)
# save acc_run and acc4odor
save(acc_run,acc4odor,accbox,file = paste0(params$path, "figures/mvpa_roi.RData"))
```
