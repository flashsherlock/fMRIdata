...
results_abs<- cbind(abs(results[,2:4]),results[,5:25])
# reverse y
...
p2 <- plot_ly(results,x=~(-x), y=~(-y), z=~z, split=~roi,type="scatter3d", mode="markers", 
        color=~`m_lim-car`,size = I(30),symbol = I("square"), hovertemplate = paste('%{marker.color:.2f}'))
...
p6 <- plot_ly(results,x=~(-x), y=~(-y), z=~z, split=~roi,type="scatter3d", mode="markers", 
        color=~(`m_lim-cit`-`m_lim-car`)/(`m_lim-cit`+`m_lim-car`),size = I(30),symbol = I("square"), hovertemplate = paste('%{marker.color:.2f}'))%>%colorbar(title = "struc-quality-norm")
