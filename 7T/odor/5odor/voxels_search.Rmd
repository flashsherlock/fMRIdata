...
names(results) <- c("sub","x","y","z","roi",paste("m",pairs,sep = "_"),paste("t",pairs,sep = "_"),diff)
results <- as.data.table(results)
# acc = 0 if below chance
temp <- results[,6:(dim(results)[2]-4)]
temp[temp<0] <- 0
results[,6:(dim(results)[2]-4)] <- temp
```
...
ui <- fluidPage(
  ...
  numericInput("t1", "t threshold", round(params$thr,6), min = 0, max = NA, step = 0.1),
  # numericInput("t2", "t_lim-car", params$thr, min = 0, max = NA, step = 0.001),
  numericInput("p1", "p value", signif(2*(1-pt(q=params$thr, df=27)),2), min = 0.00001, max = 1, step = 0.05),
