...
parfor i=1:length(rois)
    ...
    cfg.results.dir = [datafolder sub '/' sub '.' analysis '.results/mvpa/' cfg.analysis '_VIvatodor_l1_label_' num2str(shift) '/' test];
    ...
    for subi = 1:numtr
        t=tr(subi);
        F{subi} = [datafolder sub '/' sub '.' analysis '.results/'  'NIerrts.' sub '.' analysis '.odorVIvat_noblur+orig.BRIK,' num2str(t)];
    end
    cfg.files.name =  F;
    ...
    cfg.files.chunk = reshape(repmat(1:6 * 6, [1 5]), [numtr 1]);
    %
    ...
    cfg.files.label = reshape(repmat([1 2 3 4 5],[36 1]),[numtr 1]);
    % cfg.files.label = timing(:, 1);
    cfg.files.labelname = reshape(repmat({'lim' 'tra' 'car' 'cit' 'ind'},[36 1]),[numtr 1]);
