function decoding_roi_4odors_trial(sub, analysis_all, rois, shift) 
...
parfor i=1:length(rois)
    ...
    cfg.results.dir = [datafolder sub '/' sub '.' analysis '.results/mvpa/' cfg.analysis '_VIvaAM1odor_l1_label_' strrep(num2str(shift), ' ', '') '/' test];
    ...
    for subi = 1:numtr
        t=tr(subi);
        F{subi} = [datafolder sub '/' sub '.' analysis '.results/'  'NIerrts.' sub '.' analysis '.odorVIvaAM1_noblur+orig.BRIK,' num2str(t)];
    end
...
%         cfg.files.chunk = [cfg.files.chunk; reshape(repmat(1 + 48 * (shift_i - 1):shift_i * 48, [1 4]), [numtr 1])];
        cfg.files.chunk = [cfg.files.chunk; reshape(repmat(1:48, [1 4]), [numtr 1])];
        %
        ...
        cfg.files.label = [cfg.files.label;reshape(repmat([1 2 3 4], [48 1]), [numtr 1])];
        % cfg.files.label = timing(:, 1);
        cfg.files.labelname = [cfg.files.labelname;reshape(repmat({'lim' 'tra' 'car' 'cit'}, [48 1]), [numtr 1])];
    end
