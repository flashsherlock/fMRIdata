...
parfor i=1:length(rois)
    ...
    cfg.results.dir = [datafolder sub '/' sub '.' analysis '.results/mvpa/' cfg.analysis '_VIvodor_l1_label_' strrep(num2str(shift), ' ', '') '/' test];
    ...
    for subi = 1:numtr
        t=tr(subi);
        F{subi} = [datafolder sub '/' sub '.' analysis '.results/'  'NIerrts.' sub '.' analysis '.odorVIv_noblur+orig.BRIK,' num2str(t)];
    end
