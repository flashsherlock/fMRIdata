...
%     cfg.parameter_selection.parameter_range={10.^(-6:1:6);10.^(-6:1:6)};
    ...
    cfg.results.dir = [datafolder sub '/' sub '.' analysis '.results/mvpa/' cfg.analysis '_ARodor_l1_odoronly_' strrep(num2str(shift), ' ', '') '/' test];
    ...
    for shift_i=1:length(shift)
        ...
        for subi = 1:numtr
            t=timing(subi,2);
            F{subi} = [datafolder sub '/' sub '.' analysis '.results/'  'NIerrts.' sub '.' analysis '.odoronly_noblur+orig.BRIK,' num2str(t)];
