...
for (sub_i in params$sub) {
  ...
  for (op in opair) {
    # set unsignificant voxels to NA
    data_diff_i[[paste0("b_",op)]] <- ifelse(abs(data_diff_i[[paste0("t_",op)]])<t,
                                             NA,
  ...
  for (op in opair[c(1,4,5)]) {
    ...
    figure[[i]] <- plotmap(data_diff_i,paste0("b_",op),ys) +
      ...
      labs(title = paste(sub_i,opn,sep = "_"))+
...
wrap_plots(figure,nrow = 4,guides = 'collect')
```
