function decoding_roi_trial(sub,analysis_all,rois,shift,run,fname)
...
for i=1:length(comb)
    ...
    cfg.files.chunk = reshape(repmat(1:length(run)*8, [1 2]), [numtr 1]);
    %
    ...
    cfg.files.label = reshape(repmat([odornumber(1) odornumber(2)],[length(run)*8 1]),[numtr 1]);
    cfg.files.labelname = reshape(repmat({labelname1 labelname2},[length(run)*8 1]),[numtr 1]);
