...
params:
    ...
    suffix: "_tent.txt"
    rname: !r c('odor_va_trial','odor_trial')
title: "ROIs"
author: "Fei"
date: "`r Sys.time()`"
---
...
for (t in params$rname) {
  ...
  for (ir in runs) {
    ...
    datachosen <- mutate(datachosen,odor = factor(odor,levels=c("within","between")))
    # plot
  ...
  datachosen <- mutate(datachosen,roi = factor(roi,levels=params$roi,labels = params$roiname))
  datachosen <- mutate(datachosen,run = factor(run,levels=runs,label=c("all","within","between")))
  # plot
  figure <- ggplot(datachosen, aes(x=roi, y=mean, fill=run)) + 
    labs(title = t,x='ROI',y='Correlation',fill='Run')+#设置坐标轴
    geom_bar(position="dodge", stat="identity") +
    scale_y_continuous(expand = c(0,0))+
