...
# -gert_create_dataset \
# -gert_to3d_prefix yuli.run3.pa \
# -gert_outdir ../7T \
# -no_wait

# run4
# Dimon -infile_pattern '*FMRI.0020.*' \
# -gert_create_dataset \
# -gert_to3d_prefix yuli.run4.pa \
# -gert_outdir ../7T \
# -no_wait
...
# run4 完整但是没有刺激 phase correction
  afni_proc.py                                                  \
     -subj_id Yuli.pa                                              \
     -dsets yuli.run4+orig.HEAD                        \
     -blip_reverse_dset yuli.run4.pa+orig.HEAD \
     -copy_anat yuli.str+orig                                   \
     -anat_has_skull yes                                       \
     -blocks tshift align volreg blur mask scale \
     -radial_correlate_blocks tcat volreg                     \
...
# run3 不完整同时刺激不全 增加phase correction
        afni_proc.py                                                  \
        -subj_id Yuli.run3.pa                                           \
        -dsets yuli.run3+orig.HEAD                        \
        -blip_reverse_dset yuli.run3.pa+orig.HEAD \
        -copy_anat yuli.str+orig                                   \
        -anat_has_skull yes                                       \
        -blocks tshift align volreg blur mask scale regress \
        -radial_correlate_blocks tcat volreg                     \
...
# # run1 刺激完整但没有跑全 增加phase correction
        afni_proc.py                                                  \
        -subj_id Yuli.run1.pa                                         \
        -dsets yuli.run1+orig.HEAD                        \
        -blip_reverse_dset yuli.run1.pa+orig.HEAD \
        -copy_anat yuli.str+orig                                   \
        -anat_has_skull yes                                       \
        -blocks tshift align volreg blur mask scale regress \
        -radial_correlate_blocks tcat volreg                     \
