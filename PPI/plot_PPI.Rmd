...
# 箱线图和散点
figure <- ggplot(voxel,aes(x=position,y=Count)) +
  geom_boxplot()+
...
for (croi in unique(all$roi)) {
  ...
  for (con in unique(all$condition)) {
    ...
    # Error bars represent standard error of the mean
    figurelist[[i]] <- ggplot(datachosen, aes(x=position, y=value,group=interaction(position,variable),fill=variable)) +
      labs(title = title ,x='Position',y='Mean β',fill='Condition')+#设置坐标轴
      theme(axis.text.x = element_text(size=WORD_SIZE),  # 设置x轴字体大小，以下同理
            axis.text.y = element_text(size=WORD_SIZE),
            axis.title.x = element_text(size=WORD_SIZE),
      ...
      # scale_fill_manual(values = colors[1:2])+ #颜色
      scale_fill_brewer(palette = "Set2",direction = -1)+ #颜色
      geom_point(position=position_jitterdodge(jitter.width = 0, jitter.height = 0),col=2,pch=21,cex=1)+
      geom_text_repel(aes(label=Sub),position=position_jitterdodge(jitter.width = 0, jitter.height = 0))
