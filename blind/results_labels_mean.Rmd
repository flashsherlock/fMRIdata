...
# plot each odor pair
  for (m in mvpa_pattern){
    ...
    for (ana in analysis) {
      ...
      newvar[["va_within"]] <- setdiff(pairs,newvar[["valence"]])
      ...
      figure <- plot_erbar(
        mutate(subset(datachosen,odor%in%c("valence","va_within","fruf_betw","fruflo_w")),
               odor = factor(odor,
                             levels = c("valence","va_within","fruf_betw","fruflo_w"),
...
bruceR::MANOVA(subset(datasub,odor%in%c("valence","va_within")&roi%in%roiselect),subID = "sub",dv="acc",within = c("odor","roi")) %>%
  EMMEANS("odor",by = "roi", p.adjust = "fdr")
