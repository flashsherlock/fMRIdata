...
params:
    # win_path: "C:/Users/13308/Desktop/dataforR_blind/"
    # mac_path: "/Volumes/WD_F/gufei/blind/"
    ...
    sub: !r c(sprintf('S%02d',c(2:4,6:14,16:17,19:25,27,29:63,65:69))) #2:4,6:14,16:17,19:25,27,29:63,65:69
    suffix: "_tent_12.txt"
title: "ROIs"
author: "Fei&Chen"
date: "`r Sys.time()`"
...
knitr::opts_chunk$set(warning = T, message = F, cache = FALSE,tidy=F,fig.align='center',fig.showtext=TRUE,results="hold",fig.show = "hold")#fig.path = "/Volumes/WD_F/gufei/blind/figs_timecourse"
library(bruceR)
library(psych)
library(ggplot2)
library(ggthemr)
...
# 也可以是：sub <- strsplit(data$sub,fixed=T,".")
sub <- sapply(sub,"[",2)
data$sub <- sub
#fixed = T拒绝使用正则表达式
tp <- strsplit(data$TimePoints,fixed = T,"[") #如果没有fixed=T，就需要用"\\["
...
tc_smooth <- function(data_tent){
  ...
  # time_data <- zoo::rollapply(rbind(time_data[1,],time_data,time_data[nrow(time_data),]),3,mean) #fmri扫的TR是多少，时间窗就给多少
  time_data <- zoo::rollapply(rbind(time_data[1,],time_data),2,mean)#我们盲人实验的TR是2s，所以时间窗选的2。
...
# names(ratedata) <- c("sub","sex","age","blindCA","blindTP","aware",
#                     paste(rep(c("va","in","fa"),times = 6),
#                           rep(c('pin', 'app', 'ros', 'min', 'ind', 'gas'),each = 3),sep = "_"),
#                     paste("vivid",odors,sep = "_"),
#                     paste(paste0(rep("run",48),rep(1:6,each = 8)),odors,sep = "_"))#这里的vivid每个run顺序写错了，以下面的代码为准by邢晨
names(ratedata) <- c("sub","sex","age","blindCA","blindTP","aware",
                    paste(rep(c("va","in","fa"),times = 6),
                          rep(c('pin', 'app', 'ros', 'min', 'ind', 'gas'),each = 3),sep = "_"),
                    paste("vivid",odors,sep = "_"),
                    paste(paste0(rep("run",48),rep(1:6,times = 8)),rep(odors,each = 6),sep = "_"))
ratedata <- as.data.table(ratedata)
# export to spss
#bruceR::export(ratedata, file = "ratings.sav")
bruceR::export(ratedata, file = file.path(params$path,"ratings.sav"))
# melt to long format
avgdata <- separate(melt(id.vars="sub",ratedata[,c(1,7:32)]),variable,c("dimension","odor"),sep = "_")
# 建立一个list保存plotly的结果
...
for (dim in unique(avgdata$dimension)) {
  ...
  datachosen <- mutate(datachosen,odor = factor(odor,odors))
    ...
    #                                 group=odor)) + 
    figure <- ggplot(current,aes(x=odor,y=value,group=odor)) + 
    labs(title = str_to_title(dim) ,x='Odor',y=dim)+#设置坐标轴
    scale_y_continuous(expand = c(0,0))+ 
    coord_cartesian(ylim=c(1,ymax)) + 
    geom_boxplot() +
...
rundata <- separate(melt(id.vars="sub",ratedata[,c(1,33:80)]),variable,c("run","odor"),sep = "_")
...
# average vividness
ratedata[,vividavg:=rowMeans(.SD),.SDcols=c(25:32)]
...
for (sub_i in params$sub) {
  ...
  #这里需要rbind是跑循环，让每次得到的data都储存到data_tent里面，跑完最后一个循环组成的data_tent就是104个贝塔值*10个roi*62个被试的超级大表格。
  }
}
# smooth data
data_tent <- tc_smooth(data_tent)
#如果不smooth，time course的图会比较崎岖。
```
...
## Average sub
```{r}
for (region in c(params$roi,"Amy+Pir","Amy+Pir+EarlyV")) {
# choose data
  if (region =="Amy+Pir+EarlyV")
    datachosen <- subset(data_tent,roi%in%params$roi[c(1,2,7)])#1是杏仁核,2是Pir_new,7是EarlyV
  else if (region =="Amy+Pir")
    datachosen <- subset(data_tent,roi%in%params$roi[c(1,2)])
  else
