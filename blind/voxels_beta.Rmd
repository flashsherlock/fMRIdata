...
for (roi_i in names(roilist)){
  ...
  valmerge <- merge(avgvoxel,subset(avgdata,dimension=="va"))
  names(valmerge)[6] <- "rating"
  # plot
  f1 <- plot_scatter(diff_avg,"fruf","parm")+labs(title = roi_i)
  diff_avg <- merge(diff_avg,vadiff)
  f2 <- plot_scatter(diff_avg,"ratevadiff","val")
  f3 <- plot_scatter(valmerge,"rating","beta")
  print(wrap_plots(f1,f2,f3,ncol = 3))
}
```
