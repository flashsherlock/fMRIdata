...
```{r}
setwd("/Volumes/WD_D/gufei/monkey_data/campwater_newmonkey/xlsx/")
...
for (f in filename) {
  ...
  odor <- strsplit(fname[[1]][3],"")
  lf <- odor[[1]][length(odor[[1]])]
  fname <- paste(strsplit(fname[[1]][3],"[L R]"),lf,fname[[1]][1],sep = "_")
  # 用原文件名
  # 将得到的结果放到对应的变量中
