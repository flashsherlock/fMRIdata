...
class_results <- function(power_data,tar,av_freq=TRUE){
  ...
  f <- length(unique(power_by_time$time))
  resampling  <-  rsmp("cv", folds = f)
  ...
  if (dim(importance)[1]==0){
    importance <- data.table(matrix(data = 1/length(feature_names),
                                    nrow = f,ncol = length(feature_names)))
...
for (m in monkeys) {
    ...
    for (tar in tars){
      ...
      ggsave(file.path(pic_dir, paste0("Per_acc_",tar, ".pdf")), width = 3, height = 2)
      ...
      impt_per <- lapply(impts_per,colMeans)
    ...
    ggplot(data_impt[labels%in%c("true","95%"),],aes(x=frequency,y=importance))+
      ...
      scale_color_manual(values = cmap) +
      labs(title = paste(m))
    ggsave(file.path(pic_dir, paste0("Per_impt", ".pdf")), width = 3.5, height = 2)
}
```
