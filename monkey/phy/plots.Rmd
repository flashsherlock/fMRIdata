...
for (m in monkeys) {
    ...
    split <- partition(task, ratio = 55/61)
    train_idx <- split$train
    test_idx <- split$test
    task_freq <- task$clone()$select(setdiff(task$feature_names, c("roi","time")))
    # rpart classifier
    lrn <- lrn("classif.rpart")
    lrn$train(task_freq, row_ids = train_idx)
    # save plot
    ...
    dev.off()
    # test performance
    train_pre = lrn$predict(task_freq, row_ids = train_idx)
    predictions = lrn$predict(task_freq, row_ids = test_idx)
}
```
