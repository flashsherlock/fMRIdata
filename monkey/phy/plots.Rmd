...
for (m in monkeys) {
    ...
    power_noair[, roi := factor(roi, levels = as.character(1:roi_num), labels = rois)]
    power_noair[, odor := factor(odor, levels = as.character(1:5), labels = odors[1:5])]
    power_noair[, time := factor(time, levels = as.character(seq(0, 3, 0.05)))]
    ...
    for (tar in tars){
        ...
        if (tar=="odor5"){
            ...
            power_by_time[, (tar):=car::recode(odor, 'c("Ind", "Iso_l", "Iso_h") = "Neg"; c("Peach", "Banana") = "Pos"')]
        ...
        results[,`:=`(target = tar)]
        # calculate accuracy
        results <- mutate(results,acc=ifelse(results$true==results$response,1,0))
        # save to list
        ...
        ggsave(file.path(pic_dir, paste0("Acc_", tar, ".pdf")))
        # save tree plot        
        ...
        rpart.plot(learner$model,
                ...
                main = paste(m, tar, sprintf("%.2f%%",100*acc_train), sep = "_"), # title
                legend.x = 0,
                legend.y = 1,
    ...
    ggsave(file.path(pic_dir, paste0("Acc_time",".pdf")))
    # average by roi
    ...
    ggplot(mean_acc_roi, aes(x = roi, y = acc, group = target,color = target)) +
        ...
        scale_y_continuous(expand = c(0, 0), limits = c(0, 1))
    ggsave(file.path(pic_dir, paste0("Acc_roi", ".pdf")))
}
```
