...
class_results <- function(power_data,tar,av_freq=TRUE){
  ...
  importance <- matrix(nrow = length(rr$learners),
                       ncol = length(rr$learners[[1]]$importance()))
  for (cv_i in 1:length(rr$learners)) {
    importance[cv_i,] <- rr$learners[[cv_i]]$importance()
    # importance <- importance/sum(importance)
  }
  # as data.table
  importance <- as.data.table(importance)
  # set column names
  setnames(importance,new = names(rr$learners[[1]]$importance()))
  # predict labels
...
for (m in monkeys) {  
    ...
    for (tar in tars){
      # true results
      results[[m]][[tar]] <- class_results(power_data[[m]],tar,av_freq)
      ...
      results_per[[m]][[tar]] <- foreach (per_i = 1:per_num) %dopar% {
        power_per <- power_data_per[[m]][,.SD,.SDcols=c(1:4,per_i+4)]
        setnames(power_per,paste0("power",per_i),"power")
        class_results(power_per,tar,av_freq)
      }
      # stop cluster
...
for (m in monkeys) {
    for (tar in tars){
      ...
      accs <- map_depth(results_per[[m]][[tar]] , 1, "acc")
      # t-test on 61 cv
      t.test(Reduce("+", accs) / length(accs), results[[m]][[tar]][["acc"]], var.equal = T)
      # mean acc for each permutation
      acc_per <- data.frame(acc = sapply(accs,mean))
      # 95% percentile
      cri <- quantile(acc_per$acc,0.95)
      # actual acc
      tru <- mean(results[[m]][[tar]][["acc"]])
      # mean(acc_per<=tru)
      # histogram
      psize <- 2
      xrange <- c(floor(min(acc_per)*100)/100,ceiling(max(acc_per,tru)*100)/100)
      p <- ggplot(acc_per,aes(x = acc))+
